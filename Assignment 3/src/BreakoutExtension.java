 ...
 * with special extensions that enhance the user's experience.
 */
...
public class BreakoutExtension extends GraphicsProgram implements MouseMotionListener {
	...
	public static void main(String[] args) {
		new BreakoutExtension().start(args);
	}
	...
	/** Displays the title screen to the user with the title,
	 *  my name, and a copyright statement. */
	private void displayTitleScreen() {
		GLabel title = new GLabel("BREAKOUT");
		title.setFont("Helvetica-Bold-36");
		title.setColor(Color.ORANGE);
		title.setLocation((getWidth() - title.getWidth()) / 2, (getHeight() - title.getAscent()) / 2 - title.getAscent());
		add(title);

		GLabel programCreator = new GLabel("Programmed by Chirag Bharadwaj");
		programCreator.setFont("Helvetica-16");
		programCreator.setLocation((getWidth() - programCreator.getWidth()) / 2, (getHeight() - title.getAscent()) / 2 - title.getAscent() + (2 * programCreator.getAscent()));
		add(programCreator);

		GLabel copyright = new GLabel("Copyright (c) 2013 by Chirag Bharadwaj (Stanford University)");
		copyright.setFont("Helvetica-10");
		copyright.setLocation((getWidth() - copyright.getWidth()) / 2, getHeight() - copyright.getAscent());
		add(copyright);

		GLabel clickAnywhere = new GLabel("Click anywhere to start.");
		clickAnywhere.setFont("Helvetica-12");
		clickAnywhere.setColor(Color.BLUE);
		clickAnywhere.setLocation((getWidth() - clickAnywhere.getWidth()) / 2, (getHeight() - title.getAscent()) / 2 - title.getAscent() + (2 * programCreator.getAscent()) + (4 * clickAnywhere.getAscent()));
		add(clickAnywhere);
	...
	/** Sets up Breakout by creating the bricks, the paddle, and
	 *  adding the logo and score at the top of the screen. */
	...
	/* Declares the instance variable initialStart. */
	private GLabel initialStart;

	/** Creates the initial start game screen which tell the user
	 *  to click to start the game. */
	private void addInitialStart() {
		initialStart = new GLabel ("Click anywhere to start the game.");
		initialStart.setFont("Helvetica-12");
		double startHorPosition = (getWidth() - initialStart.getWidth()) / 2;
		double startVerPosition = (getHeight() - initialStart.getAscent()) / 2;
		initialStart.setLocation(startHorPosition, startVerPosition);
		initialStart.setColor(Color.BLUE);
		add(initialStart);
	}
	...
	private void createRows(double verPosition, Color color) {
		...
		for (int i = 0; i < NBRICKS_PER_ROW; i++) {			
			G3DRect brick = new G3DRect(horPosition, verPosition, BRICK_WIDTH, BRICK_HEIGHT, true);
	...
	/* Each of these are instance variable declarations. */
	private GLabel logo;
	private GLabel creator;
	private GLabel scoreKeeper;

	/** Adds Breakout logo to the top of the screen. */
	private void addLogo() {
		logo = new GLabel("Breakout");
		logo.setFont("Helvetica-Bold-24");
		logo.setColor(Color.ORANGE);
		logo.setLocation(10, logo.getAscent() + 10);
		add(logo);
	}

	/** Adds my name underneath the Breakout logo. */
	private void addCreator() {
		creator = new GLabel("Programmed by Chirag Bharadwaj");
		creator.setFont("Helvetica-12");
		creator.setLocation(10, logo.getAscent() + creator.getAscent() + 12.5);
		add(creator);
	}

	/** Adds the score label to the top right of the screen. */
	private void addScoreKeeper() {
		scoreKeeper = new GLabel("Score: " + score);
		scoreKeeper.setFont("Helvetica-Bold-12");
		scoreKeeper.setColor(Color.RED);
		scoreKeeper.setLocation(getWidth() - scoreKeeper.getWidth() - 10, 20);
		add(scoreKeeper);
	}

	/** Adds basic details on the screen for aesthetic purposes
	 *  as well as to keep track of the score. */
	private void addLogoAndScore() {
		addLogo();
		addCreator();
		addScoreKeeper();
	}
	 ...
	 *  the brick, makes a nice beep sound, adds to the score,
	 *  and bounces off. All movements are based on the laws of
	 *  physics. Also keeps track of the number of bricks left in
	 *  the grid. */
	private void checkCollider() {
		...
		} else if ((collider != null) && (collider != logo) && (collider != creator) && (collider != scoreKeeper)) {
			bounceClip.play();
			remove(scoreKeeper);
	...
	/** This method allows for the game to be played over and over
	 *  again without having to quit out of/restart the applet. */
	...
	/** Adds a label that asks the user to click to play again. */
	private void addPlayAgain() {
		GLabel playAgain = new GLabel("Click anywhere to play again.");
		playAgain.setFont("Helvetica-12");
		playAgain.setColor(Color.BLUE);
		playAgain.setLocation((getWidth() - playAgain.getWidth()) / 2, (getHeight() - playAgain.getAscent()) / 2 + (2 * playAgain.getAscent()));
		add(playAgain);
	}

...
public class Yahtzee extends GraphicsProgram implements YahtzeeConstants {
	 ...
	 */
	public void run() {
		while (true) {		// Allows Yahtzee to be played over and over again.
	...
	private void takeTurn(int round, int player) {
		...
		if (0 < category && category < UPPER_SCORE) {
			upperScore += updateScoresheet(dice, category, player, upperScore, lowerScore);
		} else if (UPPER_BONUS < category && category < LOWER_SCORE) {
			lowerScore += updateScoresheet(dice, category, player, upperScore, lowerScore);
	...
	private void rollAndRerollDice(int[] dice, int player) {
		for (int j = 1; j <= N_CONFIG; j++) {
			if (j == 1) {
				...
				for (int k = 1; k <= N_DICE; k++) {				// Randomly generates the values
					dice[k - 1] = rgen.nextInt(1, N_SIDES);		// on the five dice.
				}
			} else {
	 ...
	 *  store a value of 0.
	 */
	private int updateScoresheet(int[] dice, int category,
			int player, int upperScore, int lowerScore) {
